...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // API route for getting template-specific configuration
  app.get("/api/config/template/:templateType", async (req: Request, res: Response) => {
    try {
      const templateType = req.params.templateType;
      const validTemplateTypes = ['professionals', 'restaurants', 'tourism', 'retail', 'services'];
      
      if (!validTemplateTypes.includes(templateType)) {
        return res.status(400).json({ error: "Invalid template type" });
      }

      // Try to get existing template-specific config, or create one if it doesn't exist
      let config = await storage.getTemplateConfig(templateType);
      
      if (!config) {
        // Create default config for this template type
        const defaultConfig = {
  ...
  // API route for updating template-specific configuration
  app.put("/api/config/template/:templateType", async (req: Request, res: Response) => {
    try {
      const templateType = req.params.templateType;
      const validTemplateTypes = ['professionals', 'restaurants', 'tourism', 'retail', 'services'];
      ...
      console.log(`Updating ${templateType} template config with data:`, JSON.stringify(validationResult.data, null, 2));

      const updatedConfig = await storage.updateTemplateConfig(templateType, validationResult.data);
      if (!updatedConfig) {
        console.error(`Template configuration for ${templateType} not found`);
        return res.status(404).json({ error: "Template configuration not found" });
      }

      console.log(`Successfully updated ${templateType} template config:`, JSON.stringify(updatedConfig, null, 2));
